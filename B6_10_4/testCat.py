from cat import Cat, Wallet, Volonter
...
client_1.city = "г. Москва"
client_1.status = "Наставник"
client_2 = Wallet('Петр Иванов', 100)
client_2.city = "г. Красноярск"
client_2.status = "Студент"
...
print("Волонтеры")
vol_1 = Volonter('Сидоров Алексей', 'г. Москва', 'Спортсмен')
print(vol_1.name)

guests = [client_1, client_2, vol_1]
print("Список гостей")
for guest in guests:
    if isinstance(guest, Wallet):
        print(guest.get_client(), ',', guest.city, ', статус "', guest.status, '"')
    else:
        print(f"{guest.name}, {guest.city}, статус \"{guest.status}\"")
